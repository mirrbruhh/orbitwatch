...
for name, params in MISSIONS.items():
    ...
    dv1_m_s, dv2_m_s, total_m_s = dv1 * 1000.0, dv2 * 1000.0, total * 1000.0
    ...
    results.append((name, traj_str, total_m_s, min_exp <= total_m_s <= max_exp, nominal))

print("-" * 110)
print("\nENGINEERING STATUS & SANITY CHECKS:")
for name, traj_str, total_m_s, is_passed, expected_approx in results:
    if is_passed:
        print(f"  [PASS] {name} ({traj_str}): {total_m_s:.2f} m/s is within nominal range (~{expected_approx:.0f} m/s)")
    else:
        print(f"  [FAIL] {name} ({traj_str}): {total_m_s:.2f} m/s is OUTSIDE nominal range (~{expected_approx:.0f} m/s)")
print("=" * 110)